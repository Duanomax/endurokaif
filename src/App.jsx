        ...
        <div className="relative mx-auto max-w-7xl">
          ...
          <div className="grid gap-5 lg:grid-cols-[.82fr_1.18fr]">
            <div className="relative">
              <div className="max-h-[430px] snap-y snap-mandatory space-y-3 overflow-y-auto overscroll-contain rounded-[1.6rem] border border-white/10 bg-white/[0.03] p-2 pr-3 [scrollbar-color:rgba(34,211,238,.55)_rgba(255,255,255,.08)] [scrollbar-width:thin] md:max-h-[560px] md:rounded-[2rem] md:p-3 md:pr-4">
                {displayedFleet.map((item) => (
                  <button
                    key={item.title}
                    onClick={() => setSelected(item)}
                    className={`group flex min-h-[78px] w-full snap-start items-center justify-between gap-4 rounded-2xl border p-3 text-left transition-all md:min-h-[96px] md:rounded-3xl md:p-4 ${
                      selected.title === item.title
                        ? "border-fuchsia-400 bg-fuchsia-600 text-white shadow-[0_0_40px_rgba(217,70,239,.26)]"
                        : "border-white/10 bg-black/35 text-white hover:border-cyan-300/45 hover:bg-cyan-300/[0.08]"
                    }`}
                  >
                    <div className="min-w-0">
                      <div className="text-[10px] font-black uppercase tracking-[0.22em] text-cyan-200 md:text-xs">{item.category}</div>
                      <div className="mt-1 truncate text-lg font-black md:text-2xl">{item.title}</div>
                      <div className={`mt-1 line-clamp-1 text-xs md:text-sm ${selected.title === item.title ? "text-white/75" : "text-zinc-400"}`}>{item.subtitle}</div>
                    </div>
                    <ChevronRight className={`h-5 w-5 shrink-0 transition-transform ${selected.title === item.title ? "rotate-90 text-cyan-200" : "group-hover:translate-x-1"}`} />
                  </button>
                ))}
              </div>
              {displayedFleet.length > 5 && (
                <div className="pointer-events-none absolute inset-x-4 bottom-2 h-12 rounded-b-[1.4rem] bg-gradient-to-t from-[#050505] to-transparent" />
              )}
            </div>
